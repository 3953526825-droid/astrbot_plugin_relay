...
class RelayPlugin(Star):
    ...
    async def set_periodic_relay(
        ...
        Args:
            target_id (str): 接收者的 QQ 号、群号或别名，多个用英文逗号分隔。
            target_type (str): "group" 或 "private"。
            content (str): 传话内容，支持 {time}/{date}/{datetime} 占位符。
            interval_seconds (int): 循环间隔秒数（与其他定时参数互斥时保持 0）。
            daily_hour (int): 每日定时的整点小时（0-23），-1 表示不启用。
            daily_minute (int): 每日定时的分钟（0-59），-1 表示不启用。
            day_of_week (int): 每周定时的星期几（0=周一 ... 6=周日），-1 表示不启用。
        '''
        if not self._check_permission(event):
            return self._get_denied_msg()
    ...
    async def cancel_relay_task(
        ...
        Args:
            job_id (str): 要取消的任务 ID，格式如 "P-2"(单次定时) 或 "C-3"(周期性)，也可只填纯数字。
            task_id (str): 与 job_id 等价，二者任选其一填写即可。
            cancel_all (bool): 设为 true 则一次性清空所有任务。
        '''
        if not self._check_permission(event):
            return self._get_denied_msg()
        ...
        raw = str(job_id or task_id or "").strip()
        if not raw:
            return "未提供任务 ID。请先使用任务列表工具查看当前任务，再传入例如 job_id=\"P-2\" 来取消。"

        raw_upper = raw.upper()
        m = re.match(r"^([PC])[-_:\s]*(\d+)$", raw_upper)
        if m:
            kind = "periodic" if m.group(1) == "C" else "single"
            num = int(m.group(2))
        elif raw_upper.isdigit():
            num = int(raw_upper)
            kind = None
        else:
            return f"无法识别任务 ID '{raw}'。请使用类似 P-2（单次）或 C-3（周期）的格式。"

        if kind is None:
            if num in self.pending_tasks:
                kind = "single"
            elif num in self.periodic_tasks:
                kind = "periodic"
            else:
                return f"未找到 ID 为 {num} 的任务，请先使用任务列表工具确认。"

        if kind == "single":
            if num not in self.pending_tasks:
                return f"未找到单次定时任务 #P-{num}，请先使用任务列表工具确认。"
            info = self.pending_tasks.pop(num)
            info["task"].cancel()
            return f"已成功取消单次定时任务 #P-{num}（发往 {info['target_type']} {info['target_id']}）。"

        if num not in self.periodic_tasks:
            return f"未找到周期性任务 #C-{num}，请先使用任务列表工具确认。"
        info = self.periodic_tasks.pop(num)
        info["task"].cancel()
        return f"已成功取消周期性任务 #C-{num}（发往 {info['target_type']} {info['target_id']}）。"
